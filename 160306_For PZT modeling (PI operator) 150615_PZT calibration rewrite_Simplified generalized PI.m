...
%% 2016/02/16 To decomposite  the X(X')     %3/6 這是用在最後輸入X'
...
%plot(X_n_Given,sum(X_Calculated,2),X_n_Given,X_uniformXn);
X_n_Calculated_Sum=sum(X_n_Calculated,2);
plot(X_Given,X_n_Calculated_Sum-X_n_Calculated_Sum(1)+X_n(1),X_Given,X_n);

%% 2016/2/17 用上述function generate X_for_n和X_bak_n                    (待會順便      X_n的意思是, 如果
...
X_f_n_Calculated_Sum=sum(X_bf_n_Calculated,2);
X_bf_n_Calculated_Sum=sum(X_bf_n_Calculated,2);


plot(V_f,sum(X_f_n_Calculated,2),V_f,sum(X_bf_n_Calculated,2));
