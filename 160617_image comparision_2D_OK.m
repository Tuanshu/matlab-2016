...
ImA=sum(imread('H:\160614_CCD  phi Test\Run 2\L.bmp'),3);
%ImA=max(ImA(:))-ImA;
ImA=ImA/max(ImA(:));
...
ImB=sum(imread('H:\160614_CCD  phi Test\Run 2\R.bmp'),3);
ImB=ImB/max(ImB(:));
% %% Test- also locate A
% Pre_angle=0;
...
%%
Temp_A=edge(ImA_Blur(round((size(ImA_Blur,1)-Window_Size_X)/2)+(1:Window_Size_X),round((size(ImA_Blur,2)-Window_Size_Y)/2)+(1:Window_Size_Y)),'Canny');
Temp_B=edge(ImB_Blur(round((size(ImB_Blur,1)-Window_Size_X)/2)+(1:Window_Size_X),round((size(ImB_Blur,2)-Window_Size_Y)/2)+(1:Window_Size_Y)),'Canny');

    imwrite(Temp_A,['D:\\','Temp_A','.png']);
    imwrite(Temp_B,['D:\\\','Temp_B','.png']);
...
for p=1:length(Angle_Array)
    ImB_Rotate=imrotate(edge(ImB_Blur,'Canny'),Angle_Array(p));
...
Temp_B_record_2=circshift(Temp_B_record,[-1 -606]);
...
ylim([0 size(Comp_result,1)]);
    ...
    imwrite(Comp_result,['D:\\\','Comp_result','.png']);
