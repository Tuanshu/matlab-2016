...
Byte_Skip=0;
% Processing related
Column_Binning_Factor=2;
Y_Ave_Factor=4;
ave_factor=[4];
Product_of_Axial_Decimation_Factor_and_Ave_Factor=4;
N=4;

Center_Frequency=Frame_Rate/ave_factor/N;

Lower_Frequency=Center_Frequency*(1-Filter_Range);
Higher_Frequency=Center_Frequency*(1+Filter_Range);
...
for QQQ=1:length(ave_factor)
    ...
    if If_EffMap ==-2
        Spectrum_Image=fft(Reduced_Image,[],1);
        Frequency_Spacing=Frequency_Max/size(Spectrum_Image,1);
        Frequency_Array=Frequency_Spacing*[1:size(Spectrum_Image,1)];
        

        plot(Frequency_Array,abs(Spectrum_Image(:,320)),'LineWidth',1.5);
        xlim([0 500]);
        %ylim([0 0.1]);
        
        
        Frequency_Image=repmat(Frequency_Array',[1 size(Spectrum_Image,2)]);
        Spectrum_Image(Frequency_Image<Lower_Frequency)=0;
        Spectrum_Image(Frequency_Image>Higher_Frequency)=0;
        Reconstructed_Image=2*abs(ifft(Spectrum_Image,[],1));
        
        Temp=0;
        Axial_Length_Original=size(Reconstructed_Image,1);
        Reduced_Length=Axial_Length_Original/Axial_Bin;
        for tt=1:Axial_Bin
           Temp=Temp+Reconstructed_Image((Axial_Bin-(tt-1)):Axial_Bin:(Axial_Bin*Reduced_Length)-(tt-1),:);
        end
