...
for r=1:length(Ave_Factor_Array)
    ...
    last_folder_name='160901_for Differnt Average Test (~500 ADU)';
    ...
    Frame=Max_Frame_Number*Ave_Factor_Array(r);
    %%
...
Eff_Std_Mean_Array_fit=(FitResult.a*Eff_Mean_Array_1.^2+Eff_Mean_Array_1*(1/Max_ADU*FWC_estimated/QE)^-1+FitResult.c).^0.5;
...
plot(Ave_Factor_Array,Std_Mean_Array_2);
...
plot(Eff_Mean_Array_1,Eff_Std_Mean_Array_1,Eff_Mean_Array_1,Eff_Std_Mean_Array_fit,'-o');
xlabel('Effective Signal (ADU)');
ylabel('Effective Noise (ADU)');
xlim([0 4096]);
ylim([0 20]);
legend('Measured',sprintf('Fitted, 2nd Coef=%g',FitResult.b),'Location','NorthWest','-o');


Output=[Eff_Mean_Array_1 Eff_Std_Mean_Array_1 Eff_Std_Mean_Array_fit];

dlmwrite([Data_Save_Folder 'Output_Different Ave Factor_PCO.txt'],Output,'delimiter','\t','newline','pc','precision', '%.6f');



plot(Ave_Factor_Array,Eff_Mean_Array_1,'-o');
xlabel('Average Factor');
ylabel('Effective Signal (ADU)');
