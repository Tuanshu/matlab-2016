clear all
%%
root_folder_path='F:\160905_Imperx Noise\On System_Different Exposure Time\';

Exposure_Time_Array=[2 20:20:300];


Std_Mean_Array_1=ones([length(Exposure_Time_Array) 1]);
Mean_Array_1=ones([length(Exposure_Time_Array) 1]);
Std_Mean_Array_2=ones([length(Exposure_Time_Array) 1]);
Mean_Array_2=ones([length(Exposure_Time_Array) 1]);


for r=1:length(Exposure_Time_Array)

    last_folder_name=sprintf('%dmicrosec',Exposure_Time_Array(r));
...
%     plot(Exposure_Time_Array,Std_Mean_Array_1);
% 
%     plot(Exposure_Time_Array,Mean_Array_1);
%     plot(Exposure_Time_Array,Std_Mean_Array_1_p,Exposure_Time_Array,Std_Mean_Array_wo_Shot_1_p);

    Matrix=[Exposure_Time_Array; ones([1 length(Exposure_Time_Array)])]';
    beta=Matrix\Std_Mean_Array_wo_Shot_1_p;
    Std_Mean_Array_wo_Shot_1_p_fit=beta(1)*Exposure_Time_Array+beta(2);

%     plot(Exposure_Time_Array,Std_Mean_Array_wo_Shot_1_p,Exposure_Time_Array,Std_Mean_Array_wo_Shot_1_p_fit);
...
subplot(1,1,1)
plot(Exposure_Time_Array,Std_Mean_Array_1,Exposure_Time_Array,Std_Mean_Array_fit,'-o');
xlabel('Reference Power (mW)');
ylabel('Noise (ADU)');
legend('Measured','Fitted','Location','NorthWest');


plot(Mean_Array_1,Std_Mean_Array_1,Mean_Array_1,Std_Mean_Array_fit,'-o');
xlabel('Signal (ADU)');
ylabel('Noise (ADU)');
xlim([0 4096]);
ylim([0 20]);
...
dlmwrite([Data_Save_Folder 'Output_Different Exposure Time.txt'],Output,'delimiter','\t','newline','pc','precision', '%.6f');


plot(Exposure_Time_Array,Mean_Array_1,'-o');
