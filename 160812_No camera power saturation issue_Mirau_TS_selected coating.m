...
N=16*20000;        %入射之光子數(單位時間內, 在特定波長)
IE=0.5;           %interference efficiency

Ratio=5;     %the ratio between sample signal and sample stray light 1:ratio

Ratio_System=[0 0.3 0.6 1];

for p=1:length(Ratio_System)
    ...
    R_sample_stray=R_sample_Total*Ratio/(Ratio+1);   %assumed
    R_sample=R_sample_Total/(Ratio+1);

    % Transmittance

    T_Sample=T2.*R_sample.*T2;  %the overall T of Mirau for Sample path
    T_Reference=R2.*R1.*R2;     %the overall T of Mirau for Reference path
    T_StrayLight=T2.*R_sample_stray.*T2;  %the overall T of Mirau for Sample path
    T_StrayLight_System=R2.*T1*Ratio_System(p);  %Direct reflection from the GP2

    T_Total=T_Sample+T_Reference+T_StrayLight+T_StrayLight_System;


    N_Sample=T_Sample*N;
    N_Reference=T_Reference*N;
    N_StrayLight=T_StrayLight*N;
    N_StrayLight_System=T_StrayLight_System*N;


    N_Total=N_Sample+N_Reference+N_StrayLight+N_StrayLight_System;

    N_Interference=2*IE.*(N_Sample.*N_Reference).^0.5;

    N_Noise_Total=(N_Total).^0.5; %shot only
    N_Noise_NoSystemStray=(N_Sample+N_Reference+N_StrayLight).^0.5;
    SNR=20*log10(N_Interference./N_Noise_Total);
    SNR_NoSystemStray=20*log10(N_Interference./N_Noise_NoSystemStray);
    ...
    %%
    subplot(2,length(Ratio_System)/2,p)
