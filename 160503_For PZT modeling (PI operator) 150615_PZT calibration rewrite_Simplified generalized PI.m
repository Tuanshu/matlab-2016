...
%% 這是我的input, 重來一遍
plot(V_f,X_f,V_bf,X_bf);

plot(1:length(V_f),V_f,1:length(V_bf),V_bf) %這兩條線幾乎重疊, 所以就取長的
...
dlmwrite([Folder_path 'X_f.txt'],X_f,'delimiter','\t','newline','pc');
dlmwrite([Folder_path 'X_bf.txt'],X_bf,'delimiter','\t','newline','pc');
dlmwrite([Folder_path 'V_f.txt'],V_f,'delimiter','\t','newline','pc');
dlmwrite([Folder_path 'V_bf.txt'],V_bf,'delimiter','\t','newline','pc');
