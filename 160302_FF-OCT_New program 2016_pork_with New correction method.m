...
elseif correction_set == 2
    ...
    cmax_1=30;
    cmin_2=0.25;%12;%11
    cmax_2=0.4;%16;%60;%120;%20;
    G_Ratio=4;
    Gaussian_X_width=400*G_Ratio;%350*G_Ratio;
    Gaussian_Y_width=300*G_Ratio;%250*G_Ratio;
    X_offset=0;%160;
    Y_offset=0;%-100;
...
X_mosaic_starting_index=6;%+6+3;%6;%13;%+6;%6;%1;  %start from 0
Y_mosaic_starting_index=8;%+8+4;%+8+4;%12;%8;%17;%+8;%8;%1;
...
for tt=1:X_overlapping
    ...
    correction_A(frame_width-tt+1,:)=correction_A(frame_width-tt+1,:)*((tt-1)/((X_overlapping-1-0.5))); %原本這邊多減了0.5, 移掉15/12/30 %16/3/2 把0.5加回來
end
for tt=1:Y_overlapping
    correction_A(:,tt)=correction_A(:,tt)*((tt-1)/(Y_overlapping-1));
    correction_A(:,frame_height-tt+1)=correction_A(:,frame_height-tt+1)*((tt-1)/((Y_overlapping-1-0.5)));   %16/3/2 把0.5加上去
end
