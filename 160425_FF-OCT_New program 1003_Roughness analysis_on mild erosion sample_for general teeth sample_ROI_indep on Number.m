...
set_array=10;
for qqq=1:length(set_array)


    % Data reading & stacking
    set=set_array(qqq);
    If_load_data=1;
    If_generate_hex_map=1;
    If_select_ROI=0;
    If_Analysis=0;
    If_schematic=0;
...
%             ylim([0 size(B_scan_Adjusted,1)]);
        ...
        Reduced_Volume=permute(temp_frame_volume(:,:,Start_Searching_Index:End_Searching_Index),[2 1 3]);
            ...
            %%
            clear B_scan_Reduced
            imagesc(Reduced_Volume(:,:,66));
            
        cmin=0;%12;%11
        cmax_set=200;%16;%60;%120;%20;
    ...
    imagesc(B_scan_Reduced_Adjusted);
        ...
        dlmwrite([Training_Data_Storage_Folder_Path,sprintf('Set_%d_First_Map.txt',set)],First_Map,'delimiter','\t','newline','pc','precision', '%.6f');
        imwrite(First_Map/max(First_Map(:)),[Training_Data_Storage_Folder_Path,sprintf('Set_%d_First_Map.png',set)],'png');

        
        dlmwrite([Training_Data_Storage_Folder_Path,sprintf('Set_%d_Second_Map.png',set)],Second_Map,'delimiter','\t','newline','pc','precision', '%.6f');
        imwrite(Second_Map/max(Second_Map(:)),[Training_Data_Storage_Folder_Path,sprintf('Set_%d_Second_Map.png',set)],'png');

        
        
        dlmwrite([Training_Data_Storage_Folder_Path,sprintf('Set_%d_Ratio_Map.txt',set)],Ratio_Map,'delimiter','\t','newline','pc','precision', '%.6f');
        imwrite(Ratio_Map/max(Ratio_Map(:)),[Training_Data_Storage_Folder_Path,sprintf('Set_%d_Ratio_Map.png',set)],'png');

        
        dlmwrite([Training_Data_Storage_Folder_Path,sprintf('Set_%d_Total_Map.txt',set)],Total_Map,'delimiter','\t','newline','pc','precision', '%.6f');
        imwrite(Total_Map/max(Total_Map(:)),[Training_Data_Storage_Folder_Path,sprintf('Set_%d_Total_Map.png',set)],'png');

    else
        First_Map=dlmread([Training_Data_Storage_Folder_Path,sprintf('Set_%d_First_Map.txt',set)]);
        Second_Map=dlmread([Training_Data_Storage_Folder_Path,sprintf('Set_%d_Second_Map.txt',set)]);
        Ratio_Map=dlmread([Training_Data_Storage_Folder_Path,sprintf('Set_%d_Ratio_Map.txt',set)]);
        Total_Map=dlmread([Training_Data_Storage_Folder_Path,sprintf('Set_%d_Total_Map.txt',set)]);

        Intensity_Map=First_Map*2;
        imwrite(Intensity_Map/max(Intensity_Map(:))*2,[Training_Data_Storage_Folder_Path,sprintf('Set_%d_Intensity_Map.png',set)],'png');
    ...
    if If_select_ROI == 1
        ...
        for p=1:length(Class_index_array)
           ROI_Map(:,:,p)=roipoly(Ratio_Map)*Class_index_array(p);
        end
        ROI_Map=sum(ROI_Map,3);
        close(Fig_ROI);
        dlmwrite([Training_Data_Storage_Folder_Path,sprintf('Set_%d_ROI_Map.txt',set)],ROI_Map,'delimiter','\t','newline','pc','precision', '%.6f');

    else    
        ROI_Map=dlmread([Training_Data_Storage_Folder_Path,sprintf('Set_%d_ROI_Map.txt',set)]);
    end    

        %%
    for p=1:length(Class_index_array)
        ROI_Map_Temp=ROI_Map;
        ROI_Map_Temp(ROI_Map_Temp~=Class_index_array(p))=0;
        ROI_Map_Temp(ROI_Map_Temp==Class_index_array(p))=1;
    ...
    for p=1:kk
        ...
        Current_Data=[mean(Current_Superpixel_First) max(Current_Superpixel_First)-min(Current_Superpixel_First) mean(Current_Superpixel_Second) max(Current_Superpixel_Second)-min(Current_Superpixel_Second)];
