...
IMEC_b_Est=Fit_PCO.b*FWC_PCO_Spec/56000;
IMEC_a_Est=0;
IMEC_c_Est=Fit_PCO.c;

IMEC_Est = (IMEC_a_Est.*Output_PCO(:,1).^2+IMEC_b_Est.*Output_PCO(:,1)+IMEC_c_Est).^0.5;
%FWC_estimated_Imperx=Max_ADU/Fit_Imperx.b;
%FWC_estimated_PCO=Max_ADU/Fit_PCO.b;

plot(Output_Imperx(:,1),Output_Imperx(:,2).^2,'-o',Output_PCO(:,1),Output_PCO(:,2).^2,'-o',Output_PCO(:,1),IMEC_Est.^2,'-o');
legend('Imperx B0620','PCO HS4','IMEC solution (predicted)','Location','NorthWest');
xlim([0 3500]);
ylim([0 400]);
xlabel('Signal (ADU)');
ylabel('Noise^2 (ADU)');
