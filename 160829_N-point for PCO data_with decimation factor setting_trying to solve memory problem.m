...
Axial_Decimation_Factor=2;



Max_Number_of_Frame=10000;

Processed_Data_Path=[Data_Save_Folder last_folder_name sprintf('Axial_Decimation_Factor_%d.bin',Axial_Decimation_Factor)];
...
%%
Ave_Frame_Length=floor(Frame/ave_factor/Axial_Decimation_Factor);
...
for p=1:min(Ave_Frame_Length,Max_Number_of_Frame)
    for r=1:N
        for q=1:ave_factor
            ...
            file_path=[folder_path file_list((p-1+Start_Frame)*ave_factor*Axial_Decimation_Factor+q+Offset_1).name];
        ...
        disp(p);
        Ave_Image_Stack_Npoint(:,:,r)=((N*sum(mean(Ave_Temp,3).^2,3)-sum(mean(Ave_Temp,3),3).^2).^0.5)*(2^0.5)/N;
    end
