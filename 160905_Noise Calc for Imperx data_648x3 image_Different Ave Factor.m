...
Ave_Factor_Array=[1:10];


Std_Mean_Array_1=ones([length(Ave_Factor_Array) 1]);
Mean_Array_1=ones([length(Ave_Factor_Array) 1]);
Std_Mean_Array_2=ones([length(Ave_Factor_Array) 1]);
Mean_Array_2=ones([length(Ave_Factor_Array) 1]);


for r=1:length(Ave_Factor_Array)

    last_folder_name='On_System_Different Ave Factor';
    ...
    Frame=min(length(file_list),Max_Frame_Number*ave_factor);
    %%
...
%     plot(Ave_Factor_Array,Std_Mean_Array_1);
% 
%     plot(Ave_Factor_Array,Mean_Array_1);
%     plot(Ave_Factor_Array,Std_Mean_Array_1_p,Ave_Factor_Array,Std_Mean_Array_wo_Shot_1_p);

    Matrix=[Ave_Factor_Array; ones([1 length(Ave_Factor_Array)])]';
    beta=Matrix\Std_Mean_Array_wo_Shot_1_p;
    Std_Mean_Array_wo_Shot_1_p_fit=beta(1)*Ave_Factor_Array+beta(2);

%     plot(Ave_Factor_Array,Std_Mean_Array_wo_Shot_1_p,Ave_Factor_Array,Std_Mean_Array_wo_Shot_1_p_fit);
...
Eff_Mean_Array_1=Mean_Array_1.*Ave_Factor_Array';
Eff_Std_Mean_Array_1=Std_Mean_Array_1.*Ave_Factor_Array';
g = fittype( @(a,b,c,x) (a*x.^2+b*x+c)); %a: excess, b: shot, c: dark
FitResult = fit(Eff_Mean_Array_1, Eff_Std_Mean_Array_1.^2, g, 'StartPoint', [0.015, 0.0569, -0]);
FitResult.a
FitResult.b
FitResult.c
...
Std_Mean_Array_without_shot_1=(Eff_Std_Mean_Array_1.^2-FitResult.b*Eff_Mean_Array_1).^0.5;
%Std_Mean_Array_fit=(FitResult.a*Mean_Array_1.^2+FitResult.b*Mean_Array_1+FitResult.c).^0.5;
Std_Mean_Array_fit=(FitResult.a*Eff_Mean_Array_1.^2+Eff_Mean_Array_1*(1/Max_ADU*FWC_estimated/QE)^-1+FitResult.c).^0.5;

subplot(1,1,1)
plot(Ave_Factor_Array,Eff_Std_Mean_Array_1,Ave_Factor_Array,Std_Mean_Array_fit,'-o');
xlabel('Reference Power (mW)');
ylabel('Noise (ADU)');
legend('Measured','Fitted','Location','NorthWest');


plot(Eff_Mean_Array_1,Eff_Std_Mean_Array_1,Eff_Mean_Array_1,Std_Mean_Array_fit,'-o');
xlabel('Signal (ADU)');
ylabel('Noise (ADU)');
xlim([0 4096]);
ylim([0 20]);
...
Output=[Eff_Mean_Array_1 Eff_Std_Mean_Array_1 Std_Mean_Array_fit];

dlmwrite([Data_Save_Folder 'Output_Different Ave Factor.txt'],Output,'delimiter','\t','newline','pc','precision', '%.6f');



plot(Ave_Factor_Array,Eff_Mean_Array_1,'-o');
xlabel('Ave Factor');
ylabel('Signal (ADU)');