...
root_folder_path=['F:\Mikrotron\' Second_last_folder_name '\'];
Data_Save_Folder=['F:\Processed Data\' Second_last_folder_name '\'];

mkdir(Data_Save_Folder);

Power_Array=[1 50 150 175 200];
If_smooth = 0;
...
for r=1:length(Power_Array)
    ...
    last_folder_name=sprintf('%dmicrosec',Power_Array(r));
  ...
  %% fiel name sorting
    ...
    for p=1:Ave_Frame_Length
        for q=1:ave_factor
            ...
            Ave_Temp(:,:,q)=imread(file_path,'TIFF')';
            %fclose(fin);
        end
...
%     Ave_Image_Stack_ROI_2=Ave_Image_Stack(Row_ROI_2,Colomn_ROI_2,:);
    ...
    Ave_Image_Stack_Div_Avaliable_1=Ave_Image_Stack_Div_1(:,:,(Window_Size+1):(size(Ave_Image_Stack_ROI_1,3)-Window_Size));
    clear Ave_Image_Stack_Div_1
    Ave_Image_Stack_Avaliable_1=Ave_Image_Stack_ROI_1(:,:,(Window_Size+1):(size(Ave_Image_Stack_ROI_1,3)-Window_Size));
    Ave_Image_Stack_Mean_1=mean(Ave_Image_Stack_Avaliable_1,3);
    clear Ave_Image_Stack_Avaliable_1
    Ave_Image_Stack_Std_1=std(Ave_Image_Stack_Div_Avaliable_1,0,3); 
    clear Ave_Image_Stack_Div_Avaliable_1
...
%     Mean_Array_2(r)=Mean_2;
    h=plot(Bin_Adj_Factor*squeeze(Ave_Image_Stack_ROI_1(X_Plot,Y_Plot,:)));
    set(h,'Color',Color_Matrix(rem(r,size(Color_Matrix,1))+1,:))
    hold on
end    
    hold off
xlabel('Frame Number');
ylabel('Signal (ADU)');
title(sprintf('Signal Waveform (Different Power), X=%d, Y=%d',X_Plot,Y_Plot));
