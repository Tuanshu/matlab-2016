...
X_f_Derived_Original=(X_total_another_assumption_f-(X_f_Derived_Original(1))+X_f(1)-b_Trial)./a_Trial;
X_bf_Derived_Original=(X_total_another_assumption_bf-(X_bf_Derived_Original(1))+X_bf(1)-b_Trial)./a_Trial;
...
for p=1:length(vth_array)
    ...
    a_array(p)=mean(a_array(index_vth:index_vth_next));
    b_array(p)=mean(b_array(index_vth:index_vth_next));
