clear all
%%
root_folder_path='I:\161222_Andor iVAC';
Data_Name='161222_Andor iVAC';
Current_Array=[1];

Horizontal_Size=2000;
Vertical_Size=256;
Number_of_Frame_per_File=1000;

Size=25;


Mean_Array_Map=zeros(Vertical_Size,Horizontal_Size,length(Current_Array));
STD_Array_Map=zeros(Vertical_Size,Horizontal_Size,length(Current_Array));
Data_Stack=zeros(Vertical_Size,Horizontal_Size,Number_of_Frame_per_File);
for p=1:length(Current_Array)
    file_name=sprintf('%gA',Current_Array(p));
    ...
    cd(folder_path);
    for q=1:Number_of_Frame_per_File
        Data_Stack(:,:,q)=double(imread(file_path,q));
        disp(q)
    end
    
    Mean_Array_Map(:,:,p)=mean(Data_Stack,3);
    STD_Array_Map(:,:,p)=std(Data_Stack,1,3);
end
...
for r=1:length(Power_Array)
    ...
    last_folder_name_save=sprintf('%dmW',Power_Array(r));
    folder_path=[root_folder_path last_folder_name '\'];

    Processed_Data_Path=[Data_Save_Folder last_folder_name_save '.bin'];
    ...
    for p=1:Ave_Frame_Length
        for q=1:ave_factor
            ...
            Ave_Temp(:,:,q)=fread(fin,[Row,Colomn],'uint16')/16; %*Frame   不知為何, 看起來就像是要除16
...
%     Ave_Image_Stack_ROI_2=Ave_Image_Stack(Row_ROI_2,Colomn_ROI_2,:);
    ...
    fid = fopen([Data_Save_Folder 'Diff_Power_' last_folder_name_save '_Mean.bin'], 'w+');
    fwrite(fid, Ave_Image_Stack_Mean_1, 'double');
    fclose(fid);
    
    fid = fopen([Data_Save_Folder 'Diff_Power_' last_folder_name_save '_STD.bin'], 'w+');
